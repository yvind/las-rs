...
impl ExtraBytesReader {
    ...
    pub fn new(header: &Header) -> Self {
        ...
        for vlr in header.all_vlrs() {
            if vlr.user_id == "LASF_Spec" && vlr.record_id == 4 {
                ...
                let nr = vlr.data.len() / ExtraBytesVlr::SIZE;

                for i in 0..nr {
                    let j = i * ExtraBytesVlr::SIZE;
                    let bytes = ExtraBytesVlr {
                        _reserved: vlr.data[j..2 + j].try_into().unwrap(),
                        data_type: vlr.data[2 + j],
                        options: vlr.data[3 + j],
                        ...
                        min: vlr.data[64 + j..88 + j].try_into().unwrap(),
                        max: vlr.data[88 + j..112 + j].try_into().unwrap(),
        ...
        for (index, field) in vlrs.iter().enumerate() {
            ...
            let _ = field_name_index.insert(name, index);
        }
    ...
    pub fn value_for_named_field(&self, name: &String, point: &Point) -> f64 {
        let index = self
            .field_name_index
            .get(name)
            .unwrap_or_else(|| panic!("Unable to locate field with name '{name}'"));
...
mod tests {
    ...
    fn test_size_of_extra_bytes_vlr() {
        assert_eq!(size_of::<crate::point::extra_bytes::ExtraBytesVlr>(), 192);
    }
}
